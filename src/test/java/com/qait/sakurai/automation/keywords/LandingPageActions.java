...
public class LandingPageActions extends GetPage {
	...
	public void selectPassPointAndVerifySSO(String product) {
		...
		isElementDisplayed("lnk_NCLEX_RN",product.trim());
		//clickUsingXpathInJavaScriptExecutor(element("lnk_NCLEX_RN",product));
		element("lnk_NCLEX_RN",product).click();
		wait.hardWait(3);
		isElementDisplayed("tab_adaptive");
		element("tab_adaptive").click();
		hardWaitForIEBrowser(5);
	}
	...
	public void goToMyContentsPage() {
		...
		logMessage("[Step]: Clicked on my content tab");
		...
		logMessage("[Assertion Passed]: Verified user is on My Content page");
	}

	public void switchToNewWindowOnProd(int i) {
		if(ConfigPropertyReader.getProperty("tier").equalsIgnoreCase("prod") || ConfigPropertyReader.getProperty("tier").equalsIgnoreCase("E2E")){
			changeWindow(i);
		}
	}
